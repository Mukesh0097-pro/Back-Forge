...
export function Footer() {
    ...
    return (
        <footer className="relative bg-bg-deep border-t border-primary-blue/20">
            {/* Neon accent line */}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-8">
                    ...
                    <div className="lg:col-span-2">
                        <Link to="/" className="flex items-center space-x-2 mb-4 group">
                            <Code2 className="w-8 h-8 text-neon-cyan transition-transform duration-300 group-hover:rotate-12" />
                            ...
                            <ul className="space-y-2">
                                {links.map((link) => (
                                    <li key={link.name}>
                                        <Link
                                            to={link.href}
                                            className="text-text-secondary hover:text-neon-cyan transition-colors duration-300 text-sm"
                                        >
                                            {link.name}
                                        </Link>
                                    </li>
                ...
                {/* Bottom */}
                <div className="pt-8 border-t border-primary-blue/10">
                    <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                        <p className="text-text-secondary text-sm">
                            © {currentYear} BackForge. All rights reserved.
                        </p>
                        <p className="text-text-secondary text-sm">
                            Built with <span className="text-neon-cyan">❤</span> for developers
                        </p>
                    </div>
                </div>
            </div>
